...
$("#btn_submit").click(function(){
 ...
 // 노드 선택 시 이벤트 핸들러
   $('#menu_tree').on('select_node.jstree', function (e, data) {
		...
		if (menu_depth == '1') {
            $('#menu_depth_1').prop('checked', true);
            $('#menu_depth_2').prop('checked', false);
            $('#menu_depth_3').prop('checked', false);
        } else if(menu_depth == '2'){
            $('#menu_depth_1').prop('checked', false);
            $('#menu_depth_2').prop('checked', true);
            $('#menu_depth_3').prop('checked', false);
        }else{
			$('#menu_depth_1').prop('checked', false);
            $('#menu_depth_2').prop('checked', false);
            $('#menu_depth_3').prop('checked', true);
		}
	...
	$.ajax({
		...
		success: function(data) {
            ...
            $('#menu_tree').jstree({
                ...
                plugins: ['checkbox', 'contextmenu', 'search', 'types', 'dnd'],
				 checkbox: { //단일체크 설정
				three_state : false,
